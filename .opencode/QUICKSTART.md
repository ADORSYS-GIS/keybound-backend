# Opencode Agents & Commands - Implementation Summary
...
### 1. Agent Configurations (9 agents)
Located in `.opencode/agents/`

| Agent | LLM Model | Purpose |
|-------|-----------|---------|
| flow-orchestrator | gemini-2.5-pro | Project coordination |
| bff-generator | gemini-3.1-flash-lite | Code generation |
...
### 2. Executable Commands (7 commands)
...
- `.opencode/QUICKSTART.md` - This quick reference
...
### Modify Agent LLM
Edit agent JSON file:
```json
{
  "name": "my-agent",
  "llm": "claude-3-opus",  // Change this
  "commands": ["my-command"]
}
```
...
json_pp -t null < .opencode/agents/agent-name.json

# Verify config
json_pp -t null < .opencode/config.json
```
...
3. **Run**: `opencode run --agent flow-orchestrator daily-standup`
...
**Agent configs**: `.opencode/agents/*.json`