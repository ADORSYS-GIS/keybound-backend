...
use gen_oas_server_staff::{
    Api as StaffApi, ApiKycStaffSubmissionsGetResponse,
    ApiKycStaffSubmissionsUserIdApprovePostResponse, ApiKycStaffSubmissionsUserIdGetResponse,
    ApiKycStaffSubmissionsUserIdRejectPostResponse,
    ApiKycStaffSubmissionsUserIdRequestInfoPostResponse,
};
...
impl BffApi<ServiceContext> for BackendApi {
    async fn api_registration_kyc_documents_post(
        ...
        let s3_key = format!("kyc/{user_id}/{object_id}/{}", req.file_name);
    ...
    async fn api_registration_kyc_status_get(
        ...
        let docs = self
            .state
            .service
            .list_kyc_documents(&user_id, page, limit)
            .await
            .map_err(repo_err)?;
...
impl StaffApi<ServiceContext> for BackendApi {
    ...
    async fn api_kyc_staff_submissions_user_id_approve_post(
        ...
        _context: &ServiceContext,
    ) -> std::result::Result<ApiKycStaffSubmissionsUserIdApprovePostResponse, ApiError> {
        let req: staff_map::KycApprovalRequest = kyc_approval_request.into();
        let updated = self
            .state
            .service
            .update_kyc_approved(&user_id, &req)
            .await
            .map_err(repo_err)?;
        if !updated {
            return Ok(ApiKycStaffSubmissionsUserIdApprovePostResponse::ValidationFailed);
        }
        self.state.invalidate_bff_cache(&user_id);
        Ok(ApiKycStaffSubmissionsUserIdApprovePostResponse::KYCApproved)
    }
    ...
    async fn api_kyc_staff_submissions_user_id_get(
        ...
        limit: Option<i32>,
        _context: &ServiceContext,
    ) -> std::result::Result<ApiKycStaffSubmissionsUserIdGetResponse, ApiError> {
        let (page, limit) = Self::normalize_page_limit(page, limit);
        let profile = self
            .state
            .service
            .get_kyc_submission(&user_id)
            .await
            .map_err(repo_err)?;
        let Some(profile) = profile else {
            return Ok(ApiKycStaffSubmissionsUserIdGetResponse::SubmissionNotFound);
        };
        ...
        Ok(ApiKycStaffSubmissionsUserIdGetResponse::DetailedSubmission(
            dto.into(),
        ))
    }
    ...
    async fn api_kyc_staff_submissions_user_id_reject_post(
        ...
        _context: &ServiceContext,
    ) -> std::result::Result<ApiKycStaffSubmissionsUserIdRejectPostResponse, ApiError> {
        let req: staff_map::KycRejectionRequest = kyc_rejection_request.into();
        let updated = self
            .state
            .service
            .update_kyc_rejected(&user_id, &req)
            .await
            .map_err(repo_err)?;
        if !updated {
            return Ok(ApiKycStaffSubmissionsUserIdRejectPostResponse::ValidationFailed);
        }
        self.state.invalidate_bff_cache(&user_id);
        Ok(ApiKycStaffSubmissionsUserIdRejectPostResponse::KYCRejected)
    }
    ...
    async fn api_kyc_staff_submissions_user_id_request_info_post(
        ...
        _context: &ServiceContext,
    ) -> std::result::Result<ApiKycStaffSubmissionsUserIdRequestInfoPostResponse, ApiError> {
        let req: staff_map::KycRequestInfoRequest = kyc_request_info_request.into();
        let updated = self
            .state
            .service
            .update_kyc_request_info(&user_id, &req)
            .await
            .map_err(repo_err)?;
        if !updated {
            return Ok(ApiKycStaffSubmissionsUserIdRequestInfoPostResponse::ValidationFailed);
        }
        self.state.invalidate_bff_cache(&user_id);
        Ok(ApiKycStaffSubmissionsUserIdRequestInfoPostResponse::AdditionalInfoRequested)
    }
}
...
impl KcApi<KcContext> for BackendApi {
    ...
    async fn disable_user_device(
        ...
        if row.status != "ACTIVE" {
            return Ok(gen_oas_server_kc::DisableUserDeviceResponse::DeviceCannotBeDisabledInItsCurrentState(kc_error("INVALID_STATE", "Device cannot be disabled")));
        }
        let updated = self
            .state
            .service
